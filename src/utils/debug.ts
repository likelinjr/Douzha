...
import { RESET, TOOL_COLOR, COMMON_COLOR, REASONING_COLOR, COLORS } from "../config/themes/colors.js"
...
export function toolLog(...args: any[]): void {
  if (!DEBUG) return
  process.stdout.write(`${TOOL_COLOR}${formatArgs(args)}${RESET}`)
}
export function commonLog(...args: any[]): void {
  if (!DEBUG) return
  process.stdout.write(`${COMMON_COLOR}${formatArgs(args)}${RESET}`)
}
export function errorLog(...args: any[]): void {
  if (!DEBUG) return
  process.stdout.write(`${COLORS.APPLE_RED}${formatArgs(args)}${RESET}`)
}
export function warnLog(...args: any[]): void {
  if (!DEBUG) return
  process.stdout.write(`${COLORS.ORANGE}${formatArgs(args)}${RESET}`)
}
export function Log(color?: string, ...args: any[]): void {
  if (!DEBUG) return
  process.stdout.write(`${color || RESET}${formatArgs(args)}${RESET}`)
}
export function model_reasoning(reasoning: string): void {
  if (!DEBUG) return
  process.stdout.write(`${REASONING_COLOR}${reasoning}${RESET}`)
}
export function model_content(content: string): void {
  if (!DEBUG) return
  process.stdout.write(`${RESET}${content}${RESET}`)
}
